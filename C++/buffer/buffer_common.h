...
template<class T>
class fmt_buffer : public fmt::detail::buffer<T>
{
public:
	fmt_buffer() {}
...
//-----------------------------------------------------------------------------
// 1) 
#ifdef _CGDK_ENABLE_BUFFER_BOUND_CHECK
...
template<class B, class... T> class serializer_peek<B, std::tuple<T...>>
							{	public:	using type = std::tuple<T...>;
								...
								constexpr static std::size_t  _get_extract_size(const buffer_view& _buffer, int64_t& _offset) { CGDK_ASSERT(false); return 0;}
							};
...
std::size_t _Xsprintf(char* _dest, std::size_t /*_max_length_in_word*/, const char* _format, F&& _first, TREST&&... _rest)
{
	#if defined(_MSC_VER)
...
template <class F, class... TREST>
std::size_t _Xsprintf(wchar_t* _dest, std::size_t _max_length_in_word, const wchar_t* _format, F&& _first, TREST&&... _rest)
{
	#if defined(_MSC_VER)
	#pragma warning(disable:4996)
	#endif
