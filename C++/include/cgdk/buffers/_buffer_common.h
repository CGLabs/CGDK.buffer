...
namespace std
{
	//template<class __TA, std::size_t __TB> class array;
...
{
public:
	constexpr skip(std::size_t _amount) noexcept : amount(_amount) {}
public:
	std::size_t	amount;
};
...
{
public:
	constexpr offset(std::size_t _amount) noexcept : skip(_amount) {}
};
...
{
public:
	POS() noexcept : offset(0) {}
	POS(const POS& p) noexcept : offset(p.offset) {}
	explicit POS(int64_t p) noexcept : offset(p) {}
public:
	int64_t	get_pos() const noexcept { return offset; }
	void	set_pos(int64_t _offset) noexcept { offset = _offset; }

	POS& operator=(const POS& _rhs) noexcept { offset += _rhs.offset; return *this; }
	POS& operator=(int64_t _rhs) noexcept { offset += _rhs; return *this; }

	bool	operator==(const POS& _rhs) const noexcept { return offset == _rhs.offset; }
	bool	operator==(int64_t _rhs) const noexcept { return offset == _rhs; }
	bool	operator!=(const POS& _rhs) const noexcept { return offset != _rhs.offset; }
	bool	operator!=(int64_t _rhs) const noexcept { return offset != _rhs; }
	bool	operator>=(const POS& _rhs) const noexcept { return offset >= _rhs.offset; }
	bool	operator>=(int64_t _rhs) const noexcept { return offset >= _rhs; }
	...
	operator int64_t& () { return offset; }
	operator const int64_t& () const { return offset; }
public:
	int64_t	offset;
};
...
template<class T>	class serializer_size_of<T, std::enable_if_t<is_buffer_type_v<T>>>
							{
								...
								template<class S> 
								constexpr static std::size_t  _get_extract_size(const S& _buffer, int64_t& _offset)				{ CGDK_ASSERT_ERROR; return 0;}
							};
...
template<>					class serializer_size_of<WEB>
							{	public:
								constexpr static std::size_t _get_append_size(const WEB&)									{ CGDK_ASSERT_ERROR; return 0; }
								template<class S> 
								static constexpr std::size_t _get_extract_size(const S& _buffer, int64_t& _offset)			{ CGDK_ASSERT_ERROR; return 0;}
							};
...
template<>					class serializer_size_of<skip>
							{	public:
								constexpr static std::size_t  _get_append_size(const skip& _object)							{ return _object.amount; }
								template<class S> 
								constexpr static std::size_t  _get_extract_size(const S& _buffer, int64_t& _offset)			{ CGDK_ASSERT_ERROR; return 0;}
							};
...
template<class B, class... T> class serializer_peek<B, std::tuple<T...>>
							{	public:	using type = std::tuple<T...>;
								...
								constexpr static std::size_t  _get_extract_size(const buffer_view& _buffer, int64_t& _offset) { CGDK_ASSERT_ERROR; return 0;}
							};
...
template<class B, class T>	class serializer_append<B, T, std::enable_if_t<std::is_base_of_v<Ibuffer_serializable, T>>>
							{	using TX = std::remove_const_t<T>;
								public:	using type = _buffer_view<typename B::element_t>;
								template<class S> constexpr static type _do_append(S& _s, const TX& _data)					{ auto len_old = _s.size(); const_cast<TX&>(_data).process_serialize_out(_s); return type(_s.data() + len_old, _s.size() - len_old);}
								template<class S> constexpr static type _do_append(S& _s, TX&& _data)						{ auto len_old = _s.size(); _data.process_serialize_out(_s); return type(_s.data() + len_old, _s.size() - len_old);}
							};
template<class B, class T>	class serializer_extract<B, T, std::enable_if_t<std::is_base_of_v<Ibuffer_serializable, T>>>
							{	using TX = std::remove_const_t<T>;
								public:	using type = TX;
								template<class S> constexpr static type _do_extract(S& _s)									{ TX tx; tx.process_serialize_in(_s); return tx;}
								template<class D, class S> constexpr static void _do_extract(D& _dest, S& _s)				{ _dest.process_serialize_in(_s);}
							};
template<class B, class T>	class serializer_peek<B, T, std::enable_if_t<std::is_base_of_v<Ibuffer_serializable, T>>>
							{	using TX = std::remove_const_t<T>;
								public:	using type = T;
								template<class S> constexpr static type _do_peek(const S& _s, int64_t& _offset)				{ S tb =_s + static_cast<std::size_t>(_offset); type tx; tx.process_serialize_in(tb); return tx;}
								template<class D, class S> constexpr static void _do_peek(D& _dest, const S& _s, int64_t& _offset) { S tb =_s + static_cast<std::size_t>(_offset); _dest.process_serialize_in(tb);}
							};
...
template<class B, class T>	class serializer_append<B, T*, std::enable_if_t<std::is_base_of_v<Ibuffer_serializable, T>>>
							{	using TX = std::remove_const_t<T>;
								public:	using type = _buffer_view<typename B::element_t>;
								template<class S>
								constexpr static type _do_append(S& _s, const TX* _data)									{ auto len_old = _s.size(); if(_data!=nullptr) {const_cast<std::decay_t<TX>*>(_data)->process_serialize_out(_s);} else { _s.template _append<COUNT_T>(COUNT_T(0)-1);} return type(_s.data() + len_old, _s.size() - len_old);}
							};
template<class T>			class serializer_size_of<T*, std::enable_if_t<std::is_base_of_v<Ibuffer_serializable, T>>>
							{	using TX = std::remove_const_t<T>;
								public:
...
template<class B, class T>	class serializer_append<B, object_ptr<T>, std::enable_if_t<std::is_base_of_v<Ibuffer_serializable, T>>>
							{	using TX = std::remove_const_t<T>;
								public:	using type = _buffer_view<typename B::element_t>;
								template<class S> constexpr static type _do_append(S& _s, const TX* _data)					{ auto len_old = _s.size(); if(_data != nullptr) {const_cast<TX*>(_data)->process_serialize_out(_s);} else { _s.template _append<COUNT_T>(COUNT_T(0)-1);} return type(_s.data() + len_old, _s.size() - len_old);}
							};
template<class B, class T>	class serializer_extract<B, object_ptr<T>, std::enable_if_t<std::is_base_of_v<Ibuffer_serializable, T>>>
							{	using TX = std::remove_const_t<T>;
								...
								template<class S> constexpr static type _do_extract(S& _s)									{ object_ptr<TX> t = make_object<TX>(); t->process_serialize_in(_s); return t;}
								template<class D, class S> constexpr static void _do_extract(D& _dest, S& _s)				{ _dest = make_object<TX>(); _dest->process_serialize_in(_s);}
							};
...
template<class B, class T>	class serializer_extract<B, own_ptr<T>, std::enable_if_t<std::is_base_of_v<Ibuffer_serializable, T>>>
							{	using TX = std::remove_const_t<T>;
								...
								template<class S> constexpr static type _do_extract(S& _s)									{ own_ptr<TX> t = make_own<TX>(); t->process_serialize_in(_s); return t;}
								template<class D, class S> constexpr static void _do_extract(D& _dest, S& _s)				{ _dest = make_own<TX>(); _dest->process_serialize_in(_s);}
							};
...
template<class B, class T>	class serializer_extract<B, std::shared_ptr<T>, std::enable_if_t<std::is_base_of_v<Ibuffer_serializable, T>>>
							{	using TX = std::remove_const_t<T>;
								...
								template<class S> constexpr static type _do_extract(S& _s)									{ std::shared_ptr<TX> t = std::make_shared<TX>(); t->process_serialize_in(_s); return t;}
								template<class D, class S> constexpr static void _do_extract(D& _dest, S& _s)				{ _dest = std::make_shared<TX>(); _dest->process_serialize_in(_s);}
							};
...
template<class T>			class serializer_size_of<T, std::enable_if_t<is_linear_object_ptr_container_with_reserve<T>::value>>
							{	public:
								...
								template<class S> 
								constexpr static std::size_t  _get_extract_size(const buffer_view& _buffer, int64_t& _offset)
								{
									CGDK_ASSERT_ERROR;
									return 0;
								}
